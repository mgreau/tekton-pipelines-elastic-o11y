...
[source,shell]
--
docker run --mount type=bind,src="$(pwd)",dst=/target -it --rm docker /bin/sh -c "docker login && cp ~/.docker/config.json /target/config.json"
--
...
You can follow the logs in Kibana
...
task.pipeline.knative.dev/kaniko-push created
taskrun.pipeline.knative.dev/kaniko-pushrun created

kubectl get pods -w -n kaniko-tutorial
NAME                        READY   STATUS      RESTARTS   AGE
kaniko-pushrun-pod-e5ec3d   0/1     Completed   0          44s
--
